...
$(document).ready(function() {
	...
	$("#spartaga").click(function(){
		window.location = "./projects/spartaga.html";
	});
	...
	$("#bellhapp").click(function(){
		window.location = "./projects/bellhapp.html";
	});
		...
		if (!animationDone && !atTop) {
			...
			if (mobile){
				$("#logo-wrapper").velocity({
				    "width":$("#topbar-wrapper").height() - (screenHeight / 100) + "px", "left":0 + "px", 
	...
	function TopNav(){
		if (mobile) {
			...
			//topbar
			$("#topbar-wrapper").css({"height": $("#button-height").height() + (2 * (screenHeight / 65)) + "px", "opacity": ".5"});

			//contact buttons
			$("#contact-buttons").removeAttr( 'style' );

			$("#contact-buttons").css({"height":"5%", "left":"50%", "transform":"translateX(-50%)",
				"top":(screenHeight / 65) + "px", "text-align":"right"});
