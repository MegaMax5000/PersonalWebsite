"use strict";

$(document).ready(function() {

	//portfolio item functionality

	$("#vp").click(function(){
		window.location = "virtualPhone.html";
	});

	$("#spartaga").click(function(){
		window.location = "spartagaMain.html";
	});

	$("#space").click(function(){
		window.location = "spaceGame.html";
	});

	$("#wizard").click(function(){
		window.location = "wizard.html";
	});

	$("#kayak").click(function(){
		window.location = "kayak.html";
	});

	$("#bellhapp").click(function(){
		window.location = "bellhappMain.html";
	});

	$("#fragments").click(function(){
		window.location = "fragments.html";
	});

	$("#hackathon").click(function(){
		window.location = "hackathon.html";
	});

	$("#lunarlander").click(function(){
		window.location = "lunarlander.html";
	});

	$("#imadatavis").click(function(){
		window.location = "imadatavis.html";
	});

	$("#quadcopter").click(function(){
		window.location = "quadcopter.html";
	});

});